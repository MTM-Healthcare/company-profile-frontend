...
const ALL_CATEGORY = { id: 0, name: 'Semua Layanan', iconPath: '', order: -1 };
...
export default function AllHealthServicesPage() {
    ...
    const filteredServices = services.filter(svc => {
        const matchesCategory = selectedCategoryId === 0 || svc.categoryId === selectedCategoryId;
        const matchesSearch = svc.title.toLowerCase().includes(searchQuery.toLowerCase()) || svc.description.toLowerCase().includes(searchQuery.toLowerCase());
        return matchesCategory && matchesSearch;
    });

    const sidebarCategories = allCategories.map(cat => {
        const count = cat.id === 0
            ? services.length
            : services.filter(s => s.categoryId === cat.id).length;
        return { ...cat, count, active: selectedCategoryId === cat.id };
    });
                            ...
                            {sidebarCategories.map(cat => (
                                <li key={cat.id}>
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2.5 rounded-lg text-[13px] font-semibold transition-colors flex justify-between items-center ${cat.active
                    ...
                    <div className="w-full md:hidden flex overflow-x-auto gap-2 pb-2 hide-scrollbar">
                        {sidebarCategories.map(cat => (
                            <button
                                key={cat.id}
                                onClick={() => setSelectedCategoryId(cat.id)}
                                className={`shrink-0 px-4 py-2 rounded-full text-[12px] font-semibold transition-colors border ${cat.active
                    ...
                    <div className="flex-1 w-full">
                        {isLoading ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                                {[...Array(6)].map((_, i) => (
                                    <div key={i} className="bg-white rounded-[16px] p-5 shadow-sm border border-gray-100 h-[200px] animate-pulse" />
                                ))}
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                                {filteredServices.map((svc) => (
                                    <div key={svc.id} className="bg-white rounded-[16px] p-5 shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-shadow">
                                        <div className="w-10 h-10 rounded-xl bg-[#EEF2FF] flex items-center justify-center mb-4 relative overflow-hidden">
