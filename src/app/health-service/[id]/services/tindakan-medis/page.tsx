...
export default function TindakanMedisSelectionPage() {
    const router = useRouter();
    const params = useParams() as { id: string };

    const tindakans = [
        { name: 'Luka dan Jahitan', icon: Scissors },
        { name: 'Insisi & Bedah Minor', icon: Crosshair },
        { name: 'Perawatan Luka', icon: HeartHandshake },
        { name: 'Tindakan Umum', icon: UserCircle2 },
        { name: 'Perawatan Luka Bakar', icon: Flame },
    ];
            ...
            <main className="flex-1 pb-24 w-full relative z-10 flex flex-col items-center overflow-hidden">
                ...
                <div className="relative z-10 w-full pt-32 max-w-7xl mx-auto px-[45px]">
                    ...
                    <div className="flex flex-col mb-8 w-full max-w-[900px] mx-auto">
                        <Link href={`/health-service/${params.id}/services`} className="flex items-center gap-2 text-[#1a1a1a] hover:text-[#98141F] transition-colors mb-2 w-fit group">
                    ...
                    <div className="w-full max-w-[900px] mx-auto bg-white rounded-[16px] shadow-sm border border-gray-200 p-6 md:p-10 mb-8 mt-4">
                        ...
                        <p className="text-[14px] text-gray-600 mb-8 font-medium">
                            Silakan pilih jenis tindakan medis yang sesuai dengan kebutuhan perawatan Anda.
                        </p>
                        ...
                        <div className="relative w-full mb-12">
                            ...
                            <input 
                                type="text"
                                placeholder="Cari Layanan Tindakan Medis.."
                        ...
                        {/* Sub-Service Icons Grid */}
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 md:gap-4 pb-4">
                            {tindakans.map((service, idx) => {
                                const Icon = service.icon;
                                return (
                                    <div 
                                        key={idx} 
                                        onClick={() => {
                                            if (service.name === 'Luka dan Jahitan') {
                                                router.push(`/health-service/${params.id}/services/tindakan-medis/luka-dan-jahitan`);
                                            } else {
                                                alert("Halaman detail tindakan ini masih dalam pengembangan.");
                                            }
                                        }}
